...
/**
 * Тест оркестратора в dry-run.
 * Запуск: npx tsx src/test-process.ts [фильтр по названию]
 */
...
async function main() {
    ...
    console.log(`templatesUrl: ${v.templatesUrl || "НЕТ"}\n`);

    const result = await processVacancyResponses(
        { vacancyId: v.vacancyId, vacancyName: v.vacancyName, templatesUrl: v.templatesUrl },
        { dryRun: true }
    );
